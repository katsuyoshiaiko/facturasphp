...
function directory_list() {
    ...
    $req = $db->prepare("SELECT * FROM directory ORDER BY id DESC LIMIT $limit ");
...
."status=:status  "   
            ...
            . " WHERE id=:id ");
    $req->execute(array(
 "id" =>$id ,   "contact_id" =>$contact_id ,   "address_id" =>$address_id ,   "name" =>$name ,   "data" =>$data ,   "order_by" =>$order_by ,   "status" =>$status ,  
...
function directory_search_by_contact_id($id) {
    global $db;
    $data = null;
    $req = $db->prepare("SELECT * FROM directory WHERE contact_id = :id ");
    $req->execute(array(
        "id" => $id
    ));
    $data = $req->fetchall();
