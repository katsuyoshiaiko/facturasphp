...
class EXPENSE extends ROJO {
    ...
    function client_number() {
        ...
        $this->Cell(($this->GetPageWidth() - 20) / 100 * 30, 10, _trc('To pay') . ' ' . moneda(($expense['total'] + $expense['tax']) - abs($expense['expense'])), 1, 2, "C", 0);
    ...
    function payements_list() {
        ...
        foreach (balance_list_by_expense($expense['id']) as $key => $value) {                    
            ...
            $this->Cell(30, 5, moneda( ($expense['total'] + $expense['tva']) - abs($total_general)), 1, 0, "R", 0);
            $this->Cell(50, 5, "", 0, 1, "L", 0);                
    }
