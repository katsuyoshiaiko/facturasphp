...
function permissions_list() {
    ...
    $req = $db->prepare("SELECT * FROM permissions ORDER BY id DESC LIMIT $limit ");
...
//    $data = null;
//
//    //$req = $db->prepare("SELECT * FROM permissions ORDER BY name LIMIT $limit  ");
...
function permissions_search_by_rol($rol, $controller=null) {
    global $db;
    $data = null;
    if ($rol == "all") {
        $req = $db->prepare('SELECT * FROM permissions WHERE '
